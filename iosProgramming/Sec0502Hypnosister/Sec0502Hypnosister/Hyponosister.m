...
@interface Hyponosister()

@property (strong,nonatomic) UIColor* circleColor;

@end

@implementation Hyponosister
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    //获取三个0-1之间的数字
    float red = (arc4random() % 100) /100.f;
    float green = (arc4random() % 100) /100.f;
    float blue = (arc4random() % 100) /100.f;
    
    UIColor* randomColor = [UIColor colorWithRed:red green:green blue:blue alpha:1.0];
    self.circleColor = randomColor;
    //UIView子类，必须手动发送setNeedsDisplay消息次啊能重新绘制
...
//    [path addArcWithCenter:center radius:radius startAngle:0.f endAngle:M_PI * 2 clockwise:YES];
    ...
    CGPoint p0 = CGPointMake(center.x - 25, center.y + 25);
    CGPoint p1 = CGPointMake(center.x + 25, center.y + 25);
    CGPoint p2 = CGPointMake(center.x, center.y - 25);
...
//    [trianglePath addLineToPoint:p0];
    ...
    //RCG和一个有效位
    CGFloat components[8] = {1.0, 0.0, 1.0, 1.0, 1.0, 1.0, 0.0,1.0};
